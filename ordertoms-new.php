...
// формируем заказ для МС
    ...
    $orderMS["description"] = "Комментарий: ".$orderEcwid["orderComments"];
    $orderMS["code"] = (string)$orderEcwid["orderNumber"];
    $orderMS["name"] = $prefixNumber.(string)$orderEcwid["orderNumber"];
    if(!empty($orderEcwid["shippingOption"]["shippingMethodName"]))
        $orderMS["description"] =$orderMS["description"]."\n"."TK: ".$orderEcwid["shippingOption"]["shippingMethodName"];
    if(!empty($orderEcwid["paymentMethod"]))
        $orderMS["description"] =$orderMS["description"]."\n"."Способ оплаты: ".$orderEcwid["paymentMethod"]."\n";
/* 
<br/>
ТК: СДЭК
<br/>
Способ оплаты: На карту сбер
<br/>
 \r\n  \n
    ...
    [shippingOption] => Array
        (
            [shippingMethodId] => 4365-1553606606801
            [shippingMethodName] => Стандартная доставка
...
*/
    if(!empty($fieldMSOrder)) {
        $urlOrder = "/api/remap/1.2/entity/customerorder/metadata/attributes";
        $data = $clientMS->get($urlOrder);
        $response = json_decode($data->getBody(), true);
        $isAttribute = false;
        foreach($response["rows"] as $attribute)  {
            if($attribute["name"] == $fieldMSOrder) {
                $isAttribute = true;
                break;
            }
        }
        ...
        if(!$isAttribute) {
            ...
            "required" => false,
            "show" => true,
            "description" => "Поле создано для обмена с Эквид"
            );
            $response = $clientMS->post($urlOrder, ['json' => $newAttribute]);
            $newField = json_decode($response->getBody(), true);
            $message = " добавлено-обновлено доп.поле в заказе МС: ". $fieldMSOrder;
            writeToLog($newField,$message_prefix.$message, $level);
            $orderMS["attributes"][] = array (
                "meta" => $newField["meta"],
                "value" => (string)$orderEcwid["orderNumber"],
            );
        }
        else {
            $orderMS["attributes"][] = array (
...
//            "vat" => $item[""],
        ...
        if ($reservePositions) $position["reserve"] = $item["quantity"];
    ...
    //                writeToLog($product, 'в MS /product---');
        ...
        elseif($isBadProduct) {
          $orderMS["description"] = $orderMS["description"]."  -!-  ERROR недопустимый для обмена товар - ".$item["name"]
                ." -- сумма заказа ".$orderEcwid["total"]." руб. ";
          $isListGood = false; 
          continue;  
        }
        else {
...
// Загружаем заказ в МойСклад
    if($isListGood)  $orderMS["description"] = $orderEcwid["orderComments"];
